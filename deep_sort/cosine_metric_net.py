...
class CosineMetricNet(nn.Module):
    ...
    def _cosine_distance(a: torch.Tensor, b: torch.Tensor):
        ...
        return torch.ones([a.shape[i] for i in range(len(a.shape)) if i != 1]) - F.cosine_similarity(a,
                                                                                                     b)  # ignore dim 1
...
class TripletLoss(nn.Module):
    """
    Triplet Loss for Anchor, Positive and Negative Samples, Distance Measured by Cosine Similarity
    No Margin Needed since in cosine case
    """

    def __init__(self, margin=None):
        super(TripletLoss, self).__init__()
        self.margin = margin
        # self.test = nn.TripletMarginLoss()
...
class TrafficNet(nn.Module):
    def __init__(self, num_classes=4):
        ...
        self.num_classes = num_classes
        self.net = nn.Sequential(
            Conv2d_with_padding(in_channels=3, out_channels=64, kernel_size=3, stride=1),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),
            Conv2d_with_padding(in_channels=64, out_channels=128, kernel_size=3, stride=1),
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2, padding=1),
            Conv2d_with_padding(in_channels=128, out_channels=256, kernel_size=3, stride=1),
