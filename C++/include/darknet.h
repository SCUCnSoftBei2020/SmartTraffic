...
// custom function for output det.txt
void my_proc_img(char* maskPath, char *datacfg, char *cfgfile, char *weightfile, char *inflrname, char *outflrname, char *outtxtname, float thresh, float hier_thresh, int stfrmcnt, int ndfrmcnt);
LIB_API void my_proc_vdo(char* json_path);
// speedEstimation.h
LIB_API void speed_estimation(char* json_path);

void output_detections(int frmcnt, char *outflrname, char *outtxtname, image im, detection *dets, int num, float thresh, char **names, image **alphabet, int classes);
void output_vdo_detections(int frmcnt, FILE* out, image im, detection *dets, int num, float thresh, char **names, int classes);

void violation_detection(char *vdo_path, char *trk_path);
// layer.h
